...
import { getLocalized, getProductSlug, getVariantSlug } from './localizedContent';
...
export function findProductByVariant(categoryKey, variantSlug, locale) {
  ...
  return products.find(
    (p) => p.type === category.type && getVariantSlug(p, locale) === variantSlug
  );
}
...
export function getCategoryStaticPaths(categoryKey) {
  ...
  for (const locale of SUPPORTED_LOCALES) {
    for (const product of categoryProducts) {
      ...
      paths.push({
        params: { variant: getVariantSlug(product, locale) },
        locale,
      });
    }
  }
